...
const GRASS_DENS={snow:0,volcano:1};
...
function buildGrass(isl,g){const B=BIOMES[isl.biome],per=GRASS_DENS[isl.biome]===undefined?6:GRASS_DENS[isl.biome];isl.gIM=null;if(!per||!isl.grass.length)return;
...
function refreshHomeGrass(){const isl=islands&&islands[0];if(!isl||!isl.gIM)return;const im=isl.gIM,per=im.userData.per;if(!isl.gHid)isl.gHid=new Map();let ch=false;
